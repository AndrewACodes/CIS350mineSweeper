...
public class MineSweeperGUI extends JFrame implements ActionListener, MouseListener{
    ...
    public MineSweeperGUI(int size) {
                           ...
                           if(ae.getSource() == buttons[x][y])
                           {
                               switch (counts[x][y]) {
                                   case MINE:
                                       ...
                                       youLose(1);                                    //user clicked on a mine
                                       break;
                                   case 0:
                                       buttons[x][y].setText(counts[x][y] +"");
                                       buttons[x][y].setEnabled(false);
                                       ...
                                       youWin(); //checks win every move
                                       break;
                                   default:
                                       buttons[x][y].setText(""+counts[x][y]);
                                       buttons[x][y].setEnabled(false);
                                       youWin();                                          // its a number > 0 and not a mine, so just check for win
                                       break;
                               }
                           }    
                       }
       ...
       public static void main(String[] args){
           new MineSweeperGUI(20);    // Can be made of any size. (For now only squares)
